...
(function (global) {
  ...
  var context = 0;
  var source = 0;
  var audioBufferSizeNoteOn = 1024;
  var audioBufferSizePlayer = 8192;
  var audioBufferSize = audioBufferSizePlayer;
  var waveBuffer;
  var midiFileBuffer;
  ...
  function noteOn_WebAudioAPI(channel, key, velocity) {
	  if (!isEndless) {
		  isEndless = true;
		  audioBufferSize = audioBufferSizeNoteOn;
		  play2_WebAudioAPI(midijs_url + '../midi/p-60.midi')
	  }
	  if (song != 0) {
  ...
  function getAbsoluteUrl(url) {
  	if (!aForAbsoluteUrl) {
  		aForAbsoluteUrl = document.createElement('a');
  	}
